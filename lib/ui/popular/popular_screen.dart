...
class _PopularScreenState extends State<PopularScreen> {
  List<dynamic> popularMusic = []; // This will hold the list of popular songs
  bool isLoading = true; // To show a loading indicator

  @override
  void initState() {
    super.initState();
    _fetchPopularSongs(); // Fetch popular songs when the screen is initialized
  }

  // Function to fetch popular songs from the backend
  Future<void> _fetchPopularSongs() async {
<<<<<<< HEAD
    final url = 'http://192.168.2.8:8000/music/popular'; // Replace with your backend URL
=======
    final url = 'http://192.168.2.7:8000/music/popular'; // Replace with your backend URL
>>>>>>> a3741b2fa2019b9d61d6c85042ebf3b16d811730
    try {
      ...
      if (response.statusCode == 200) {
        ...
        setState(() {
          popularMusic = data['popular_music']; // Assign the popular songs list
          isLoading = false; // Set loading to false when data is loaded
        });
      } else {
        // If the server did not return a 200 OK response, throw an error.
        _showErrorDialog('Failed to load popular songs: ${response.reasonPhrase}');
      }
    } catch (e) {
          ...
          children: [
            ...
            Expanded(
              child: ListView.builder(
                itemCount: popularMusic.length, // Number of popular songs
                itemBuilder: (context, index) {
                  final song = popularMusic[index];
                  return ListTile(
